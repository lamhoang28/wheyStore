...
import com.example.wheystore_nhom6.Ui.Tab.Acti_HoaDon;
...
public class hoaDon_Dao {
    ...
    public void create_HoaDon(hoaDon _hd, Context context) {
        ...
        _hd.setIdPost(documentReference.getId());
        hd.put("ID", documentReference.getId());

        hd.put("idSanPham", _hd.getIdSanPham());
        hd.put("nameSanPham", _hd.getNameSanPham());
        hd.put("imageSanPham", _hd.getImageSanPham());
        hd.put("priceSanPham", _hd.getPriceSanPham());
        hd.put("countSanPham", _hd.getCount());

        hd.put("idUser", _hd.getId());
        hd.put("nameUser", _hd.getName());
        ...
        documentReference.set(_hd).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                ...
                //Intent intent = new Intent(new Intent(context, Acti_HoaDon.class));
                ...
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        ...
                        if (task.isSuccessful()) {
                            ...
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                hoaDon hd = document.toObject(hoaDon.class);
                                if (document.getData().get("id").toString().equals(user.getEmail())) {
                                    _list.add(hd);
                                }
                            }
                            adapter.notifyDataSetChanged();
