...
public class sanPham_Dao {
    ...
    public void create_SanPham(sanPham sp, Context context) {
        ...
        sanPham.put("adress", sp.getAdress());
        sanPham.put("phone", sp.getPhone());
        sanPham.put("image", sp.getImage());
        sanPham.put("brand", sp.getBrand());
        sanPham.put("price", sp.getPrice());
        sanPham.put("weight", sp.getWeight());
        sanPham.put("element", sp.getElement());
        sanPham.put("count", sp.getCount());
        sanPham.put("unCount", sp.getUnCount());
        sanPham.put("message", sp.getMessage());
        sanPham.put("note", sp.getNote());
        sanPham.put("view", sp.getView());
        sanPham.put("maxType", sp.getMaxType());
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        ...
                        if (task.isSuccessful()) {
                            ...
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                ...
                                if (document.getData().get("index").equals(type)){
                                    _list.add(sp);
                                }
                            }
...
//                                }
        ...
        docRef.get(source).addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    update_document(index,value,docum);
        ...
        docRef.get(source).addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    DocumentSnapshot document = task.getResult();
                    String key = String.valueOf(document.getData().get(docum));
                    int value = Integer.parseInt(key)+count;
                    update_document(index,value,docum);
    ...
    public void update_document(String index, int count, String document){
        DocumentReference washingtonRef = db.collection("SanPham").document(index);
        washingtonRef
    ...
    public void UpdateView(String Id,String _document){
        DocumentReference docRef = db.collection("SanPham").document(Id);
        Source source = Source.CACHE;
        docRef.get(source).addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    DocumentSnapshot document = task.getResult();
                    String key = String.valueOf(document.getData().get(_document));
                    int value = Integer.parseInt(key)+1;
                    update_document(Id,value,_document);
